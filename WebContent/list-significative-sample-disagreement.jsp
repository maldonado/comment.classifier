...
<body>
	...
	<div class='container'>
		...
		<div class='h3'>
			<h3>Reviewer name: ${reviewerName} , reviewer classification: ${comment.reviewerClassification} </h3>
		</div>
		
		<div class='h3'>
			<h3>Everton's classification: ${comment.classification} </h3>
		</div>
	...
	<script>
		...
		$("#classification").on("selectmenuchange",	function(event, ui) {
			...
			window.location = "/comment.classifier/mvc?logic=UpdateSignificativeSampleCommentDisagreementLogic&commentId="	+ comment_id + "&classification="+ classification + "&reviewer=" + reviewer_name});

		$("#projects").button();
		$("#projects").click(function() {
			window.location = "/comment.classifier/mvc?logic=ListReviewersDisagreementLogic"
		});

		$("#previous").button();
		$("#previous").click(function() {
			window.location = "/comment.classifier/mvc?logic=ListSignificativeSampleCommentsDisagreementLogic&commentId="+ comment_id+ "&reviewerName="+ reviewer_name
		});
							...
							buttons : [
									{
										...
										click : function() {
											window.location = "/comment.classifier/mvc?logic=UpdateSignificativeSampleCommentDisagreementLogic&commentId="
													+ comment_id
